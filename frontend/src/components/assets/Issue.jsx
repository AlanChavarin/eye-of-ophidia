  ...
  return (
    <div key={issue._id} className={IssuesCSS.issueContainer}>
        ...
        {(issueData) && <>
            ...
            <div>Current Status:  <b style={{fontWeight: '600', color: '#db3c30'}}>{issue.status}</b></div>}

            {(issueData?.status === 'fixed') && 
            <div>Current Status:  <b style={{fontWeight: '600', color: '#54e356'}}>{issue.status}</b></div>}

            {(issueData?.status === 'closed') && 
            <div>Current Status:  <b style={{fontWeight: '600', color: '#5469e3'}}>{issue.status}</b></div>}
